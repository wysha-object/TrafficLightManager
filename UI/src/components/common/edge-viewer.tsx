...
import Lane from "../custom-phase-tool/lane";
...
function GetCustomPhaseLane(
    ...
    if (type == "carLane") {
        result.left = (edgeGroupMask.m_Car.m_Left.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.left;
        result.straight = (edgeGroupMask.m_Car.m_Straight.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.straight;
        result.right = (edgeGroupMask.m_Car.m_Right.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.right;
        result.uTurn = (edgeGroupMask.m_Car.m_UTurn.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.uTurn;
        result.left = (edgeGroupMask.m_Car.m_Left.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.left;
        result.straight = (edgeGroupMask.m_Car.m_Straight.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.straight;
        result.right = (edgeGroupMask.m_Car.m_Right.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.right;
        result.uTurn = (edgeGroupMask.m_Car.m_UTurn.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.uTurn;
    ...
    if (type == "publicCarLane") {
        result.left = (edgeGroupMask.m_PublicCar.m_Left.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.left;
        result.straight = (edgeGroupMask.m_PublicCar.m_Straight.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.straight;
        result.right = (edgeGroupMask.m_PublicCar.m_Right.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.right;
        result.uTurn = (edgeGroupMask.m_PublicCar.m_UTurn.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.uTurn;
        result.left = (edgeGroupMask.m_PublicCar.m_Left.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.left;
        result.straight = (edgeGroupMask.m_PublicCar.m_Straight.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.straight;
        result.right = (edgeGroupMask.m_PublicCar.m_Right.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.right;
        result.uTurn = (edgeGroupMask.m_PublicCar.m_UTurn.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.uTurn;
        result.left = displayOptions.publicCarLane.left ? result.left : "none";
        result.straight = displayOptions.publicCarLane.straight ? result.straight : "none";
    ...
    if (type == "trackLane") {
        result.left = (edgeGroupMask.m_Track.m_Left.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.left;
        result.straight = (edgeGroupMask.m_Track.m_Straight.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.straight;
        result.right = (edgeGroupMask.m_Track.m_Right.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.right;
        result.left = (edgeGroupMask.m_Track.m_Left.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.left;
        result.straight = (edgeGroupMask.m_Track.m_Straight.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.straight;
        result.right = (edgeGroupMask.m_Track.m_Right.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.right;
        result.left = displayOptions.trackLane.left ? result.left : "none";
        result.straight = displayOptions.trackLane.straight ? result.straight : "none";
        result.right = displayOptions.trackLane.right ? result.right : "none";
        result.uTurn = "none";
    }
    if (type == "pedestrianLaneStopLine") {
        result.all = (edgeGroupMask.m_PedestrianStopLine.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.all;
    }
    if (type == "pedestrianLaneNonStopLine") {
        result.all = (edgeGroupMask.m_PedestrianNonStopLine.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.all;
    }
    return result;
}
...
export default function EdgeViewer(
    ...
    const displayCarLane = displayOptions.carLane.left || displayOptions.carLane.straight || displayOptions.carLane.right || displayOptions.carLane.uTurn;
    const displayPublicCarLane = displayOptions.publicCarLane.left || displayOptions.publicCarLane.straight || displayOptions.publicCarLane.right || displayOptions.publicCarLane.uTurn;
    const displayTrackLane = displayOptions.trackLane.left || displayOptions.trackLane.straight || displayOptions.trackLane.right;

    if (!displayCarLane && !displayPublicCarLane && !displayTrackLane && !displayOptions.pedestrianLaneStopLine && !displayOptions.pedestrianLaneNonStopLine) {
        return <></>;
    }
    ...
    return (
        <LaneContainer>
            {displayCarLane && <>
                ...
                {(displayPublicCarLane || displayTrackLane || displayOptions.pedestrianLaneStopLine || displayOptions.pedestrianLaneNonStopLine) && <Divider />}
            </>}
            {displayPublicCarLane && <>
                <Column>
                    <Lane
                        data={GetCustomPhaseLane(data, index, "publicCarLane", displayOptions)}
                        index={index}
                        showIcon={true}
                        onClick={clickHandler}
                    />
                </Column>
                {(displayTrackLane || displayOptions.pedestrianLaneStopLine || displayOptions.pedestrianLaneNonStopLine) && <Divider />}
            </>}
            {displayTrackLane && <>
                <Column>
                    <Lane
                        data={GetCustomPhaseLane(data, index, "trackLane", displayOptions)}
                        index={index}
                        showIcon={true}
                        onClick={clickHandler}
                    />
                </Column>
                {(displayOptions.pedestrianLaneStopLine || displayOptions.pedestrianLaneNonStopLine) && <Divider />}
            </>}
            {cityConfiguration.leftHandTraffic && <>
                {displayOptions.pedestrianLaneStopLine && <>
                    <Column>
                        <Lane
                            data={GetCustomPhaseLane(data, index, "pedestrianLaneStopLine", displayOptions)}
                            index={index}
                            showIcon={true}
                            onClick={clickHandler}
                        />
                    </Column>
                    {displayOptions.pedestrianLaneNonStopLine && <Divider />}
                </>}
                {displayOptions.pedestrianLaneNonStopLine && <>
                    <Column>
                        <Lane
                            data={GetCustomPhaseLane(data, index, "pedestrianLaneNonStopLine", displayOptions)}
                            index={index}
                            showIcon={true}
                            onClick={clickHandler}
                        />
            ...
            {!cityConfiguration.leftHandTraffic && <>
                {displayOptions.pedestrianLaneNonStopLine && <>
                    ...
                    {displayOptions.pedestrianLaneStopLine && <Divider />}
                </>}
                {displayOptions.pedestrianLaneStopLine && <>
                    <Column>
                        <Lane
