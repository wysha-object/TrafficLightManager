...
function GetCustomPhaseLane(
    ...
    if (type == "carLane") {
        result.left = (subLaneGroupMask.m_Car.m_Left.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.left;
        result.straight = (subLaneGroupMask.m_Car.m_Straight.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.straight;
        result.right = (subLaneGroupMask.m_Car.m_Right.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.right;
        result.uTurn = (subLaneGroupMask.m_Car.m_UTurn.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.uTurn;
        result.left = (subLaneGroupMask.m_Car.m_Left.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.left;
        result.straight = (subLaneGroupMask.m_Car.m_Straight.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.straight;
        result.right = (subLaneGroupMask.m_Car.m_Right.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.right;
        result.uTurn = (subLaneGroupMask.m_Car.m_UTurn.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.uTurn;
    ...
    if (type == "trackLane") {
        result.left = (subLaneGroupMask.m_Track.m_Left.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.left;
        result.straight = (subLaneGroupMask.m_Track.m_Straight.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.straight;
        result.right = (subLaneGroupMask.m_Track.m_Right.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.right;
        result.left = (subLaneGroupMask.m_Track.m_Left.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.left;
        result.straight = (subLaneGroupMask.m_Track.m_Straight.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.straight;
        result.right = (subLaneGroupMask.m_Track.m_Right.m_YieldGroupMask & (1 << index)) != 0 ? "yield" : result.right;
    ...
    if (type == "pedestrianLaneStopLine") {
        result.all = (subLaneGroupMask.m_Pedestrian.m_GoGroupMask & (1 << index)) != 0 ? "go" : result.all;
    }
    return result;
}
...
export default function SublaneViewer(
    ...
    const clickHandler = useCallback((index: number, type: CustomPhaseLaneType, direction: CustomPhaseLaneDirection, currentSignal: CustomPhaseSignalState) => {
        if (subLaneUpdateHandler) {
            ...
            const newGroupMask: SubLaneGroupMask = JSON.parse(JSON.stringify(subLane));
            if (type == "carLane") {
                if (direction == "left") {
    ...
    if (!displayCarLane && !displayTrackLane && !displayOptions.pedestrianLaneStopLine) {
        return <></>;
    }
    ...
    return (
        <LaneContainer>
            {displayCarLane && <>
                <Column>
                    <Lane
                        data={GetCustomPhaseLane(subLane, index, "carLane", displayOptions)}
                        index={index}
                        showIcon={true}
                        onClick={clickHandler}
                    />
                </Column>
                {(displayTrackLane || displayOptions.pedestrianLaneStopLine) && <Divider />}
            </>}
            {displayTrackLane && <>
                <Column>
                    <Lane
                        data={GetCustomPhaseLane(subLane, index, "trackLane", displayOptions)}
                        index={index}
                        showIcon={true}
                        onClick={clickHandler}
                    />
            ...
            {displayOptions.pedestrianLaneStopLine && <>
                <Column>
                    <Lane
                        data={GetCustomPhaseLane(subLane, index, "pedestrianLaneStopLine", displayOptions)}
                        index={index}
                        showIcon={true}
                        onClick={clickHandler}
                    />
