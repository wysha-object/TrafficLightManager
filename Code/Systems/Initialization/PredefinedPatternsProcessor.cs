...
public class PredefinedPatternsProcessor
{
    ...
    )
    {
        ...
        for (int i = 0; i < connectedEdges.Length; i++)
        {
            ...
            for (int j = 0; j < subLanes.Length; j++)
            {
                ...
                if (!job.m_LaneSignalData.TryGetComponent(subLane, out var laneSignal) || !job.m_TrackLaneData.HasComponent(subLane))
                {
                    continue;
                }
                if (job.m_CarLaneData.HasComponent(laneConnectionMap[subLane].m_SourceSubLane))
    ...
    )
    {
        ...
        for (int i = 0; i < subLanes.Length; i++)
        {
            ...
            bool isTrackLane = job.m_TrackLaneData.TryGetComponent(subLane, out var trackLane);
    ...
    )
    {
        for (int i = 0; i < subLanes.Length; i++)
        {
            ...
            if (!job.m_CarLaneData.TryGetComponent(subLane, out var carLane) && !job.m_TrackLaneData.HasComponent(subLane))
            {
                continue;
            }
            ...
            if (!job.m_LaneOverlapLookup.TryGetBuffer(subLane, out var laneOverlapBuffer) || laneOverlapBuffer.Length == 0)
            {
            ...
            else
            {
                ...
                foreach (var laneOverlap in laneOverlapBuffer)
                {
                    ...
                    if (!job.m_CarLaneData.TryGetComponent(overlapSubLane, out var overlapCarLane) && !job.m_TrackLaneData.HasComponent(overlapSubLane))
                    {
                        continue;
                    }
        ...
        for (int i = 0; i < subLanes.Length; i++)
        {
            ...
            else
            {
                ...
                foreach (var laneOverlap in laneOverlapBuffer)
                {
                    ...
                    bool isTrackLane = !job.m_TrackLaneData.TryGetComponent(overlapSubLane, out var overlapTrackLane);
                    if (!job.m_LaneSignalData.TryGetComponent(overlapSubLane, out var overlapLaneSignal))
                    {
                        continue;
                    }
    ...
    public static void ResetExtraLaneSignal(ref InitializeTrafficLightsJob job, DynamicBuffer<SubLane> subLanes, ref TrafficLights trafficLights)
    {
        for (int i = 0; i < subLanes.Length; i++)
        {
            ...
            if (!job.m_ExtraLaneSignalData.TryGetComponent(subLane, out var extraLaneSignal))
            {
                continue;
            }
