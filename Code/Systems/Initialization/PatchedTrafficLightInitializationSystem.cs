...
public partial class PatchedTrafficLightInitializationSystem : Game.GameSystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        JobChunkExtensions.ScheduleParallel(
            new InitializeTrafficLightsJob
            {
                ...
                m_CustomTrafficLightsType = InternalCompilerInterface.GetComponentTypeHandle(
                    ref __TypeHandle.__CustomTrafficLights_RW_ComponentTypeHandle,
                ...
                m_LaneLookup = InternalCompilerInterface.GetComponentLookup(ref __TypeHandle.__Game_Net_Lane_RO_ComponentLookup, ref base.CheckedStateRef),
                ...
                m_ExtraLaneSignalData = InternalCompilerInterface.GetComponentLookup(ref __TypeHandle.__ExtraLaneSignal_RW_ComponentLookup, ref base.CheckedStateRef),
                ...
                m_EdgeGroupMaskType = InternalCompilerInterface.GetBufferTypeHandle(ref __TypeHandle.__EdgeGroupMask_RW_BufferTypeHandle, ref base.CheckedStateRef),
                m_SubLaneGroupMaskType = InternalCompilerInterface.GetBufferTypeHandle(ref __TypeHandle.__SubLaneGroupMask_RW_BufferTypeHandle, ref base.CheckedStateRef),
                ...
                m_LaneOverlapLookup = InternalCompilerInterface.GetBufferLookup(ref __TypeHandle.__Game_Net_LaneOverlap_RO_BufferLookup, ref base.CheckedStateRef),
                m_ConnectedEdgeLookup = InternalCompilerInterface.GetBufferLookup(ref __TypeHandle.__Game_Net_ConnectedEdge_RO_BufferLookup, ref base.CheckedStateRef),
                ...
                m_CustomPhaseDataLookup = InternalCompilerInterface.GetBufferLookup(ref __TypeHandle.__CustomPhaseData_RO_BufferLookup, ref base.CheckedStateRef),
    ...
    public struct InitializeTrafficLightsJob : IJobChunk
    {
        ...
        [ReadOnly]
        public ComponentLookup<Lane> m_LaneLookup;
        ...
        public BufferTypeHandle<EdgeGroupMask> m_EdgeGroupMaskType;

        public BufferTypeHandle<SubLaneGroupMask> m_SubLaneGroupMaskType;

        [ReadOnly]
        public BufferLookup<SubLane> m_SubLaneLookup;

        [ReadOnly]
        public BufferLookup<LaneOverlap> m_LaneOverlapLookup;
        ...
        public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
        {
            ...
            BufferAccessor<ConnectedEdge> connectedEdgeAccessor = chunk.GetBufferAccessor(ref m_ConnectedEdgeType);
            BufferAccessor<EdgeGroupMask> edgeGroupMaskAccessor = chunk.GetBufferAccessor(ref m_EdgeGroupMaskType);
            BufferAccessor<SubLaneGroupMask> subLaneGroupMaskAccessor = chunk.GetBufferAccessor(ref m_SubLaneGroupMaskType);
            ...
            for (int i = 0; i < nativeArray3.Length; i++)
            {
                ...
                CustomTrafficLights customTrafficLights =
                    i < customTrafficLightsArray.Length ? customTrafficLights = customTrafficLightsArray[i] : new CustomTrafficLights(CustomTrafficLights.Patterns.ModDefault);
                ...
                {
                    customPhaseDataBuffer = m_CustomPhaseDataLookup[trafficLightGroupEntity];
                }
                ...
                if (i < edgeGroupMaskAccessor.Length && i < subLaneGroupMaskAccessor.Length && customPhaseDataBuffer.HasValue)
                {
                    CustomPhaseUtils.ValidateBuffer(
                        ref this,
                        entityArray[i],
                        subLanes,
                        connectedEdgeAccessor[i],
                        edgeGroupMaskAccessor[i],
                        subLaneGroupMaskAccessor[i],
                        m_SubLaneLookup
                    );
                    CustomPhaseProcessor.ProcessLanes(
                        ...
                        edgeGroupMaskAccessor[i],
                        subLaneGroupMaskAccessor[i],
                        customPhaseDataBuffer.Value
                    );
                }
                else
                {
                    var edgeInfoArray = NodeUtils
                        .GetEdgeInfoList(Allocator.Temp, entityArray[i], ref this, subLanes, connectedEdgeAccessor[i], edgeGroupMaskAccessor[i], subLaneGroupMaskAccessor[i])
                        .AsArray();
        ...
        private bool FindMoveableBridgeData(Entity node, Entity owner, out MoveableBridgeData moveableBridgeData)
        {
            ...
            if (!m_SubObjectLookup.TryGetBuffer(entity, out var bufferData))
            {
                return false;
            }
        ...
        private void ProcessMoveableBridgeLanes(
            Entity entity,
            NativeList<LaneGroup> vehicleLanes,
            NativeList<LaneGroup> pedestrianLanes,
            NativeList<LaneGroup> groups,
            DynamicBuffer<SubLane> subLanes,
            MoveableBridgeData moveableBridgeData,
            bool isSubNode,
            ref NativeHashMap<PathNode, int> groupIndexMap,
            out int groupCount
        )
        {
            if (groupIndexMap.IsCreated)
            ...
            EdgeIterator edgeIterator = new EdgeIterator(Entity.Null, entity, m_ConnectedEdgeLookup, m_EdgeData, m_TempData, m_HiddenData);
            EdgeIteratorValue value;
            while (edgeIterator.GetNext(out value))
            {
                if (m_SubLaneLookup.TryGetBuffer(value.m_Edge, out var bufferData))
                {
                    ...
                    {
                        Lane lane = m_LaneLookup[bufferData[i].m_SubLane];
            ...
            for (int j = 0; j < vehicleLanes.Length; j++)
            {
                ...
                if (!groupIndexMap.TryGetValue(m_LaneLookup[subLanes[value2.m_LaneRange.x].m_SubLane].m_StartNode, out value2.m_GroupIndex))
                {
            ...
            for (int k = 0; k < pedestrianLanes.Length; k++)
            {
                ...
                {
                    if (!groupIndexMap.TryGetValue(m_LaneLookup[subLanes[value3.m_LaneRange.x].m_SubLane].m_StartNode, out value3.m_GroupIndex))
                    {
        ...
        )
        {
            ...
            for (int j = 0; j < pedestrianLanes.Length; j++)
            {
                ...
                if (!value2.m_IsUnsafe && m_LaneOverlapLookup.HasBuffer(subLane))
                {
                    DynamicBuffer<LaneOverlap> dynamicBuffer = m_LaneOverlapLookup[subLane];
    ...
    private struct TypeHandle
    {
        ...
        public ComponentLookup<ExtraLaneSignal> __ExtraLaneSignal_RW_ComponentLookup;
        ...
        public BufferTypeHandle<EdgeGroupMask> __EdgeGroupMask_RW_BufferTypeHandle;

        public BufferTypeHandle<SubLaneGroupMask> __SubLaneGroupMask_RW_BufferTypeHandle;
        ...
        public BufferLookup<CustomPhaseData> __CustomPhaseData_RO_BufferLookup;
        ...
        public void __AssignHandles(ref SystemState state)
        {
            ...
            __ExtraLaneSignal_RW_ComponentLookup = state.GetComponentLookup<ExtraLaneSignal>();
            ...
            __EdgeGroupMask_RW_BufferTypeHandle = state.GetBufferTypeHandle<EdgeGroupMask>();
            __SubLaneGroupMask_RW_BufferTypeHandle = state.GetBufferTypeHandle<SubLaneGroupMask>();
            ...
            __CustomPhaseData_RO_BufferLookup = state.GetBufferLookup<CustomPhaseData>(isReadOnly: true);
        }
    }
