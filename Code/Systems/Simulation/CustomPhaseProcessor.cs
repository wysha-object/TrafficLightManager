    ...
    )
    {
        ...
        for (int i = 0; i < subLanes.Length; i++)
        {
            ...
            if (CustomPhaseUtils.TryGet(edgeGroupMasks, sourceEdge, edgePosition, out EdgeGroupMask groupMask) >= 0)
            {
                ...
                if (job.m_TrackLaneData.TryGetComponent(subLane, out var trackLane))
                {
            ...
            if (job.m_ExtraLaneSignalData.HasComponent(subLane))
            {
        ...
        for (int i = 0; i < subLanes.Length; i++)
        {
            ...
            laneSignal.m_GroupMask = ushort.MaxValue;
            laneSignal.m_Default = 0;
            if (job.m_LaneOverlapLookup.HasBuffer(subLane))
            {
                bool hasCarLane = false;
                foreach (var overlap in job.m_LaneOverlapLookup[subLane])
                {
                    ...
                    if (!job.m_TrackLaneData.HasComponent(overlap.m_Other))
                    {
                        continue;
                    }
